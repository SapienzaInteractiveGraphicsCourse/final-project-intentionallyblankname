...
<tr><td>✅</td><td>Refactor: <code>main.js</code> (~2650 righe) diviso in 5 moduli — <code>mathUtils.js</code>/<code>debugPanelHelpers.js</code> (helper puri), <code>MainMenu.js</code>/<code>BallPossession.js</code>/<code>ShootingSystem.js</code> (pattern "context object", zero import circolari — vedi CLAUDE.md → "Architettura: Refactor Modulare"); <code>main.js</code> ridotto a ~1710 righe. Passata <code>/simplify</code> post-refactor: rimosso un destructure morto (<code>get: getBasketball</code>), unificate 3 copie della stessa "incolla palla al ballRestPoint" e 2 copie dello stesso clamp orbit-pitch in helper condivisi, sostituito un escape-hatch <code>getScore()</code> con un vero <code>resetScore()</code>, ridotta un'allocazione per-tick nel palleggio a 120Hz</td></tr>
...
### Modularizzazione (main.js → 5 moduli)

Dopo lo scouting su come altri repo del corso condividono stato tra moduli (vedi "Confronto con Altri Progetti" più sotto), `main.js` è stato diviso in `mathUtils.js`/`debugPanelHelpers.js`/`MainMenu.js`/`BallPossession.js`/`ShootingSystem.js` (pattern "context object", vedi CLAUDE.md per il dettaglio architetturale). Problemi reali incontrati durante lo split:
