...
<tr><td>🟨</td><td><b>Section 3 — Enemies & Polish</b></td><td>Enemy 1v1 con AI tattica (<code>EnemyAI.js</code>) ✅, Steal/Block (<code>CombatMoves.js</code>) ✅ — 3v3 (più avversari) ancora da fare. Personalizzazione menu colori, animation tweaks, secondo polishing e allineamento con esame restano da fare (Main Menu/HUD rework già anticipato in Section 2)</td></tr>
...
<tr><td>✅</td><td>Refactor: stato di tiro/camera/menu consolidato in oggetti (<code>shootingState</code>, <code>cameraState</code>, <code>menuState</code>) invece di una trentina di <code>let</code> sciolti a modulo — stesso principio del punto sopra, nessun cambio di comportamento; passata dead-code/DRY (un <code>export</code> superfluo su <code>RIM_TUBE_RADIUS</code> tolto, nessun'altra dichiarazione/classe CSS morta trovata)</td></tr>
...
<tr><td>✅</td><td>Refactor: <code>main.js</code> (~2650 righe) diviso in 7 moduli — <code>mathUtils.js</code>/<code>constants.js</code>/<code>debugPanelHelpers.js</code>/<code>debugPanel.js</code> (helper/pannello debug), <code>MainMenu.js</code>/<code>BallPossession.js</code>/<code>ShootingSystem.js</code> (pattern "context object", zero import circolari — vedi CLAUDE.md → "Architettura: Refactor Modulare"); <code>main.js</code> ridotto a ~1520 righe, con un unico <code>gameContext</code> condiviso da 4 moduli invece di ricopiare gli stessi campi in 4 letterali paralleli. Tre passate <code>/simplify</code> (diff + intera codebase, 4 agenti paralleli ciascuna): rimosso un destructure morto (<code>get: getBasketball</code>), unificate copie duplicate di "incolla palla al ballRestPoint"/clamp orbit-pitch/inviluppo audio in helper condivisi, sostituito un escape-hatch <code>getScore()</code> con un vero <code>resetScore()</code>, una closure ricreata ad ogni chiamata in <code>CollisionWorld.resolve()</code> (hot path 120Hz/240Hz/2400×frame) trasformata in metodo vero, allocazioni evitabili nella preview di traiettoria sostituite con scratch riusati — vedi "Modularizzazione" più sotto per il dettaglio del bug getter-vs-spread scoperto lungo il percorso</td></tr>
<tr><td>✅</td><td>Collisione fisica robot-vs-robot (non le combat move — il semplice non-compenetrarsi) basata sul possesso della palla: chi ce l'ha assorbe solo il 25% della separazione, chi non ce l'ha il 75% ("chi viene addosso cede il passo"), non più simmetrica né condizionata al muoversi con WASD</td></tr>
<tr><td>✅</td><td>Terza passata <code>/simplify</code> (dopo STEAL/BLOCK/EnemyAI): un bug vero trovato dai revisori (non solo stile) — <code>stealState.contactMade</code> mai resettato a inizio reach né nel reset da menu del nemico, un furto interrotto a metà con contatto già avvenuto faceva "riuscire" il tentativo successivo senza un vero contatto; condizione di rubabilità ritypata due volte unificata in <code>canStealFrom()</code>; fallback <code>getAimYaw</code> ripetuto unificato in <code>resolveAimYaw()</code>; guard "combat move in corso" (ridigitato in due file, mancante su un terzo writer dello stesso joint) generalizzato in <code>isCombatMoveActive()</code> esportata; <code>DEFEND_OFFSET_DISTANCE</code> reso relazione esplicita invece di magic number indipendente; radice quadrata ridondante nella correzione fisica evitata (gate <code>lengthSq()</code> + <code>divideScalar</code> invece di un secondo <code>normalize()</code>)</td></tr>
<tr><td>✅</td><td>Section 3: Enemy 1v1 — <code>EnemyAI.js</code>, FSM tattica <code>EnemyState</code> (CHASE_BALL/ATTACK/DEFEND) ricalcolata ogni frame da chi possiede la palla, navigazione <code>moveToward</code>/<code>faceToward</code> (muove verso un punto potendo guardarne un altro, usato da DEFEND per marcare tra giocatore e canestro), tiro risolto con <code>solveBallisticElevation</code>. 3v3 (più avversari, differenziazione tra classi diverse) resta da fare</td></tr>
<tr><td>⬜</td><td>Section 3: personalizzazione menu colori</td></tr>
<tr><td>✅</td><td>Section 3: STEAL — <code>CombatMoves.js</code>, sweep della base con contatto tracciato durante tutta la reach (non trasferito al primo tocco) e possesso trasferito solo a fine animazione con le condizioni rivalutate fresche, cooldown 10s + lockout vittima 2s</td></tr>
<tr><td>✅</td><td>Section 3: BLOCK — <code>CombatMoves.js</code>, arm punta la palla in volo per tutta la reach, devia un <code>FREE_SHOT</code> senza cambiare owner</td></tr>
...
### Modularizzazione (main.js → 7 moduli)

Dopo lo scouting su come altri repo del corso condividono stato tra moduli (vedi "Confronto con Altri Progetti" più sotto), `main.js` è stato diviso in `mathUtils.js`/`constants.js`/`debugPanelHelpers.js`/`debugPanel.js`/`MainMenu.js`/`BallPossession.js`/`ShootingSystem.js` (pattern "context object", vedi CLAUDE.md per il dettaglio architetturale). Problemi reali incontrati durante lo split e i giri di pulizia successivi:

| # | Problema | Causa | Soluzione |
|---|---|---|---|
| 1 | Impossibile spostare direttamente in un modulo nuovo nessuno dei ~15 `let` tunabili da pannello debug (es. `DRIBBLE_PUSH_DURATION`) | Gli ES module creano binding di **sola lettura** sui named import: un `export let X` non è riassegnabile dal modulo che lo importa, anche se chi lo esporta può farlo — gli `onChange` degli slider (`X = v`) avrebbero smesso di funzionare | Consolidati tutti gli scalari tunabili collegati in un unico oggetto MUTABILE per sistema (`dribbleTuning`, `handlingTuning`, `shootTuning`) PRIMA di ogni split — la mutazione di una proprietà tramite reference importata è sempre permessa, solo il rebinding dell'import non lo è |
| 2 | Dopo aver spostato `stepDribble` in `BallPossession.js` e riscritto tutte le chiamate in `main.js`, la preview robot del Main Menu continuava a chiamare `stepDribble(...)` senza che il nome fosse più importato — nessun errore da `node --check`/`npm run build` (controllano solo sintassi/risoluzione moduli, non se un identificatore bare esiste davvero a runtime dentro una funzione mai eseguita durante il build) | Un'istruzione `import` dimenticata durante il refactor manuale — lo stesso tipo di bug già visto nella tabella "Main Menu" qui sopra (`poleMetalMaps`), stavolta durante uno split di file invece che un refactor inline | Catturato rileggendo manualmente ogni call-site rimasta con `grep` dopo lo spostamento, non dal build — checklist esplicita da ripetere ad ogni file spostato: cercare tutte le chiamate al nome esportato, non solo quelle nel file appena scritto |
| 3 | Un `const { get: getBasketball, ... } = ctx` in `ShootingSystem.js`, seguito da un `void getBasketball` per silenziare il lint, non faceva assolutamente nulla | `ctx.basketball` è un **getter** chiamato `basketball` (`get basketball() {...}` nell'oggetto letterale passato da `main.js`), non una proprietà letteralmente chiamata `get` — destrutturare `get` non intercetta il getter, cattura sempre `undefined` | Trovato dalla passata `/simplify` post-refactor (agenti paralleli reuse/simplification/altitude, tutti e tre l'hanno segnalato indipendentemente); rimosso, il resto del file già usava correttamente `ctx.basketball` diretto ovunque |
| 4 | Nello stesso giro, la soluzione "giusta" al #3 (spostare `get basketball() {...}` una volta sola in un context condiviso `gameContext`, invece di ripeterlo in 4 punti) introduceva un bug nuovo, catturato PRIMA di applicarlo | Lo spread `{ ...gameContext }` valuta subito qualunque accessor `get x() {...}` presente e ne congela il risultato CORRENTE in una proprietà statica sul nuovo oggetto — verificato con un test isolato (`{...{get v(){return x}}}.v` non segue più aggiornamenti di `x` dopo lo spread). `basketball` è assegnato in modo asincrono al caricamento del GLTF (è `null` all'avvio): un `get basketball` dentro `gameContext` avrebbe congelato `null` per sempre in tutti e 4 i consumer | `getBasketball: () => basketball` come **funzione** (non un accessor `get`) dentro `gameContext`: una proprietà il cui valore è una funzione sopravvive allo spread (copia il riferimento alla funzione, non il suo risultato) — stesso principio già in uso per `getBallRadius`/`getCrosshairHeight`, esteso al caso che aveva fatto inciampare il #3 |
| 5 | `CollisionWorld.resolve()` (hot path: 120Hz palleggio, 240Hz volo di tiro, fino a 2400×frame nella preview di traiettoria) ricreava una closure `resolveBox` ad ogni singola chiamata | Una `const resolveBox = (box, restitution) => {...}` dichiarata dentro il metodo, non sul prototype — pressione GC pura in un hot path, trovata da una passata `/simplify` sull'intera codebase (non solo il diff recente) | Trasformata in un vero metodo di classe (`resolveBoxAt(box, restitution, position, velocity, dt, cooldownMap, ballRadius)`), zero allocazione per chiamata |
| 6 | Stesso giro: `updateTrajectoryPreview` allocava una `new Map()` per i cooldown e un `.clone()` per il punto "precedente" ad ogni chiamata (fino a 2400 volte per frame mentre si mira) | La preview ha sempre avuto una Map/scratch "nuovi ad ogni chiamata" per non condividere stato col volo reale — corretto per la SEPARAZIONE, ma non serviva RICREARLI, solo svuotarli | Map riusata (`.clear()` invece di `new Map()`), scratch Vector3 riusato (`.copy()` invece di `.clone()`) per il solo punto "precedente" — i punti pushati nell'array della curva restano `.clone()`ati, quelli sì necessari (ogni punto della curva deve essere un oggetto distinto) |

### Enemy 1v1 e Combat Moves (STEAL/BLOCK)

| # | Problema | Causa | Soluzione |
|---|---|---|---|
| 1 | Dopo un pickup/STEAL, la paletta a volte restava visivamente "storta" durante il palleggio successivo, si "aggiustava" da sola solo mirando un istante in HANDLING | `resetToNeutralPossession()` azzerava la VARIABILE tracciata (`handlingState.tiltOffset = 0`) ma non chiamava mai `setShootTilt(0)` sul joint vero — il palleggio automatico non tocca mai il tilt da solo, quindi il joint restava al valore lasciato da un HANDLING precedente finché qualcos'altro (solo HANDLING) non lo riscriveva | Aggiunta la chiamata esplicita `manipulator.controls.setShootTilt(0)` mancante (e, per coerenza/difesa, `setDribbleOffsets(0,0)` esplicito anche dove già provabilmente ridondante) |
| 2 | STEAL trasferiva il possesso al PRIMO contatto, tagliando corto lo sweep proprio nel momento in cui aveva successo — visivamente "a scatto" | Il trasferimento avveniva dentro il check di collisione della fase `reach`, non a fine animazione | Il contatto è ora solo TRACCIATO (`stealState.contactMade`) durante lo sweep; il possesso passa solo a `t>=1`, con le condizioni di rubabilità rivalutate fresche in quel momento (non assunte ancora valide da quando il contatto è avvenuto) |
| 3 | Bug introdotto dal fix precedente, trovato da una passata `/simplify` (non dall'utente): un furto interrotto a metà reach (es. BACK TO MAIN MENU) con contatto già avvenuto faceva "riuscire" il tentativo SUCCESSIVO istantaneamente, senza un vero contatto | `stealState.contactMade` non veniva mai resettato né a inizio di un nuovo `reach` né nel reset da menu del nemico | Reset esplicito di `contactMade` a inizio di ogni `startReach()` (copre qualunque causa di interruzione precedente) + aggiunto anche al reset menu per difesa in profondità |
| 4 | Con lo sweep ora a durata piena (0.4-0.6s, non più istantaneo), il robot continuava a camminare/girarsi mentre il braccio faceva l'animazione per conto suo — sembrava un unico glitch | Né `EnemyAI.js` né il movimento WASD del giocatore sapevano che una combat move era in corso, continuavano a prendere decisioni di movimento in parallelo | Guard condiviso `isCombatMoveActive(stealState, blockState)`, esportato da `CombatMoves.js` e usato per congelare movimento/decisioni in tutti e tre i punti che scrivono sullo stesso joint (`EnemyAI.js`, WASD, e il tracking camera→braccio del giocatore in `main.js` — quest'ultimo "funzionava" solo per un ordine di chiamata fortunato in `animate()`, non per una vera protezione) |
| 5 | L'angolo di mira "flickerava" (scattava di colpo) subito dopo uno STEAL riuscito, se nel frattempo si era mosso il mouse | Il resolve di STEAL interpolava l'angolo verso `manipulator.wheelsGroup.rotation.y` — corretto per il nemico (la sua mira segue le ruote) ma sbagliato per il giocatore, la cui mira in dribble segue la CAMERA, scollegata dalle ruote | Sweep pivot e bersaglio di rientro del resolve ora usano `getAimYaw()` (per il giocatore: `cameraState.orbitYaw`; per il nemico: fallback sulle ruote), letto fresco ogni frame invece di un valore congelato o della fonte sbagliata |
| 6 | Lo sweep di STEAL a volte partiva "di lato" rispetto a dove puntava il crosshair | Il pivot dello sweep partiva da `wheelsGroup.rotation.y` (direzione dell'ultimo movimento WASD) — in `NO_BALL` la camera orbita libera indipendente da dove ci si è mossi per ultimo | Stessa soluzione del #5: pivot iniziale letto da `getAimYaw()` |
| 7 | Il nemico in DEFEND non si posizionava stabilmente tra giocatore e canestro nonostante il codice di posizionamento fosse corretto geometricamente | Il bersaglio di marcatura (`DEFEND_OFFSET_DISTANCE`) era stato messo deliberatamente SOTTO la distanza minima anti-compenetrazione (`AI_MIN_PLAYER_DISTANCE`) per dare una sensazione di "pressione" — creava invece un tiro alla fune ogni frame tra l'IA (che spingeva verso il bersaglio troppo vicino) e la correzione fisica (che respingeva al minimo consentito), impedendo la convergenza | `DEFEND_OFFSET_DISTANCE` reso una relazione esplicita `AI_MIN_PLAYER_DISTANCE + margine` (appena sopra, non sotto) — il bersaglio è sempre una posizione fisicamente valida, la correzione non deve mai intervenire |
| 8 | `#combat-panel` e `#scoreboard` non si nascondevano MAI dal menu nonostante il toggle JS della classe `.hidden` | Stesso bug già visto per `#menu-overlay` (vedi CLAUDE.md): entrambi gli elementi impostano `display: flex` direttamente sulla propria regola `#id` (per il loro layout interno) — un selettore ID batte sempre una classe in specificità CSS, quindi `.hidden { display: none }` non li sovrascriveva mai | Aggiunto `#combat-panel.hidden`/`#scoreboard.hidden { display: none }` (selettore composto ID+classe, specificità pari o superiore) |
| 9 | La pillola `#mode-indicator` ("MODE: SPECTATE") restava visibile anche da menu, mentre tutto il resto dell'HUD era già correttamente nascosto | Unico elemento HUD rimasto senza la classe `hidden` di default nell'HTML — dimenticato quando gli altri erano stati messi a posto | Aggiunta `hidden` di default + toggle nello stesso punto (START/BACK TO MAIN MENU) di `scoreboard`/`dashPanel`/`controlsHint` |

### Camera HANDLING, Collisioni Robot e STEAL Asimmetrico

Giro di bug report giocati dal vivo (non trovati da `/simplify`), dopo l'introduzione di STEAL/BLOCK/EnemyAI: un giro di modifiche non ancora committate aveva silenziosamente rotto la camera; due bug più vecchi (mai notati prima perché serve un secondo robot per farli emergere) sono usciti giocando 1v1 con più attenzione.

| # | Problema | Causa | Soluzione |
|---|---|---|---|
| 1 | La camera in HANDLING era tornata dritta dietro il robot invece che angolata "di spalla" | Una modifica non ancora committata aveva azzerato `HANDLING_CAMERA_SIDE_OFFSET` (da -60 a 0), con un commento che ne spiegava il perché ma non era mai stata verificata dal vivo | Ripristinato a -60 |
| 2 | Tenendo il tasto destro (HANDLING) mentre si stava mirando, il crosshair saltava a puntare altrove (es. il pavimento) invece di restare sul punto appena mirato | `cameraState.orbitPitch` veniva forzato a `ORBIT_PITCH_REST` ad ogni ingresso in HANDLING, scartando il pitch che si aveva in DRIBBLE | Rimosso il reset: il range pitch di HANDLING contiene per intero quello di DRIBBLE, non serve nessun clamp — vedi CLAUDE.md → "Section 2: Stato HANDLING" |
| 3 | Tirando subito dopo un DASH, il tiro a volte "impazziva" (deviazione/accelerazione innaturale) come se avesse colpito qualcosa che non si vedeva | I robot non erano MAI stati risolti contro `collisionWorld` (muri/pali/panchine/backboard) — solo la palla lo era. Il dash (6.6× velocità, nessun check) poteva spingere il robot dentro/oltre un palo o una panchina; la palla, che segue sempre la paletta fino al rilascio, partiva quindi già conficcata in quel box, e il primo sotto-passo del volo la risolveva subito con un rimbalzo sulla faccia più vicina | `clampRobotToCourt()`: stesso test sfera-vs-box già usato per la palla, applicato anche al robot (giocatore e nemico) ogni frame dopo il movimento — velocità fittizia a zero, solo correzione di posizione |
| 4 | Anche SENZA dash, se il nemico restava semplicemente vicino durante il windup/release di un tiro — PURE stando alle spalle del giocatore, non solo di fronte — il tiro veniva "disturbato" | La correzione anti-compenetrazione robot-vs-robot (25%/75% in base al possesso) girava ogni frame di Play senza eccezioni: chi teneva la palla (quindi anche chi stava tirando, fino al rilascio fisico) veniva comunque spostato del 25% se il nemico si avvicinava troppo — root.position pilota anche la chase camera, quindi il crosshair slittava proprio durante il rilascio. Nessuna dipendenza da davanti/dietro: è solo un check di distanza | Chi sta eseguendo il proprio tiro (`shootingState`/`enemyShootingState !== 'idle'`, windup/release/**recover**) non viene mai spostato da questa correzione; l'altro robot assorbe per intero lo shortfall |
| 5 | STEAL era rubabile anche restando alle spalle dell'avversario, controintuitivo per uno sweep che dovrebbe avere un allungo direzionale | Il test di contatto era un semplice cubo (`Box3`) espanso uniformemente in ogni direzione attorno all'avversario, indipendente da dove guardava/spazzava lo stealer | Margine asimmetrico rispetto a `resolveAimYaw()` dello stealer: pieno davanti (`STEAL_FORWARD_MARGIN=90`), quasi nullo alle spalle (`STEAL_BACKWARD_MARGIN=20`), interpolato con un coseno — vedi CLAUDE.md → "1v1: Nemico e Combat Moves" |

Aggiunto anche un tool di ispezione mai richiesto prima ma utile per verificare tutto quanto sopra a occhio: `src/CollisionDebugView.js`, wireframe toggle su tasti numerici **1-8** per tutti i volumi di collisione/contatto del gioco (backboard/ferro/muri/pali/panchine, zona STEAL asimmetrica, raggio BLOCK, zona PICKUP) — vedi CLAUDE.md → "Debug: Wireframe Collisioni".
